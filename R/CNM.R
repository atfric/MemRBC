...
{ t0=proc.time()
  cl=match.call()
  if (is.null(M$Ref)) stop("CNM needs SEN Reference; M$Ref cannot be NULL; then, to switch off SEN, set M.mu=M.Ka=0.")
...
# experimental: filter
    ...
    { SEN(A,grd,bas,Ref1,h2) -> S
      E_SEN(A,grd,bas,S,Ref1) -> w } else {w=0;S=NULL}
...
#      imag.obj.colorbar(O,f=S$alpha,clr=FALSE,par=FALSE,specular="black");rgl::title3d(paste(iter,"alpha"))
    ...
    if(file.exists("STOP_CNM.txt") ) {cat(crayon::red("exit by presence of file STOP_CNM\n"));file.remove("STOP_CNM.txt");MEx=TRUE;break;} else MEx=FALSE
