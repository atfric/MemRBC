...
#' @export
PNEM <- function( M, nsteps =100, dt =0.0005,
                  LAfreq = 100, plt =TRUE, filter_delta = ID,
                  rho  = 1, sd0 = 1e-3,
                  pltfreq = 25,
                  ncores = 4, E_crit = 10,
                  new_Av = FALSE, viscosity = 20, zero_Av = FALSE,
                  mass_update_freq = 100 ) # returns a modified MemRBC
{
  t0=proc.time()
 ...
 M.Rcpp_ncores<<-ncores # not working on Linux
  ...
  if (!is.null(M$E_total_PNEM)) M$E_total_PNEM=c(M$E_total_PNEM,E_total[1:iter]) else M$E_total_PNEM=E_total[1:iter]
  if (!is.null(M$E_kin_PNEM)) M$E_kin_PNEM=c(M$E_kin_PNEM,E_kin[1:iter]) else M$E_kin_PNEM=E_kin[1:iter]
  if (!is.null(M$type_PNEM)) M$type_PNEM=c(M$type_PNEM,type_PNEM[1:iter]) else M$type_PNEM=type_PNEM[1:iter]
  if (!is.null(M$C_PNEM)) M$C_PNEM=c(M$C_PNEM,C_PNEM[1:iter]) else M$C_PNEM=C_PNEM[1:iter]
  M$mass=mass
  M$E=E$E
  M$C=E$Curv
