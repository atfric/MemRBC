...
#'  Steepest Descend Rosen Constraint
#'
#' run SDRC on an existing membrane object to reduce energy by steepest descend.
#' Constraint violation is projected out by Rosen (1961).
...
#' @export
SDRC <- function(M,nsteps=100,del_min=1e-7,del_cons=1e-3,max_iter=10,cons_tol=1e-3,plt=FALSE,pltfreq=10,LAfreq=25,filter_strength=0.0)
{
 cl=match.call()
 run_id=rlang::hash(M)
 CIiter=0
 ...
 if(M.mu==0 & M.Ka==0) GS=list(grad_SEN=matrix(0,bas$Ai_max,3))
 ...
 for (i in 1:nsteps) {
  ...
  G  <- RosenProjection((GE$grad_SCM + GS$grad_SEN)*filter,GE,bas)
  ...
  GE <- Grad_SCM(E,grd,bas,C)
  CI <- ConsIter(A, grd, bas, C, GE, Ctol = cons_tol, del_cons = del_cons,nsteps = max_iter)
   A <- CI$A
  ...
  cat("SDRC:",i,"E",Et/M.Es,"C",E$Curv,"CN",CN[i],"del_min",del_min,"C0",M.C0,"F",filter_strength,"CI",CI$cons_iter,"\n")
  if(plt & (i%%pltfreq==0)) {rgl::clear3d();plot3b(C$X,grd);rgl::title3d(paste("SDRC",i,"E",round(Et/M.Es,4),"C",round(E$Curv,4)))}
  attr(A,"method")="SDRC"
  attr(A,"E")<-Et
  attr(A,"C")<-E$Curv
  attr(A,"C0")<-M.C0
...
M$E=E$E; M$C=E$Curv
if (is.null(M$SDRC_Sample)) M$SDRC_Sample<-data.frame(E=E_SD,A=A_SD,V=V_SD,C=C_SD,CN=CN,IC=I_SD,ID=run_id) else M$SDRC_Sample<-rbind(M$SDRC_Sample,data.frame(E=E_SD,A=A_SD,V=V_SD,C=C_SD,CN=CN,IC=I_SD,ID=run_id))
if (is.null(M$SDRCiter)) M$SDRCiter<-i else M$SDRCiter<-i + M$SDRCiter
