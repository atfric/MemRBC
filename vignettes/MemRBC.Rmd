...
knitr::opts_chunk$set(  collapse = TRUE,  comment = "#>", fig.width=800)
options(width=800)
...
: \*by `SetConstraints()` one may feed a C-constraint into membrane objects basis `$bas`
...
Users should check regularly that - besides SEN parameters - `M.C0` and constraint values in 'M\$bas\$Target' is set correctly for their App. That's why most Apps report C0 in their regular text output.

## Demonstration

You load global parameters with `load_param_MemRBC()`. There is a demo dataset M1 you can load and inspect. If you want the parameters from the membrane object `M1` to be set globally, use `SetParams(M1).`
...
save_MemRBC(M1,"M1_v95.rdat") # saves with modified $Target
...
Initial curvature C is reported to be 81.01442, and the 1056 integration points are on a 32 x 33 grid. The stored coefficients are in `M$A`, and `M$LA` is a list for keeping coefficients of further simulation steps.

```{r, prepareMMC, message=TRUE, warning=TRUE, webgl=TRUE}
MakeStandardRBC(L=7)->M
plot(M)
...
A set of 3 curvature constraint Metropolis-Montecarlo simulations is performed with increase of target curvature by 1 unit per cycle. in M\$LA every 10 accepted steps, coefficients are recorded. 100 steps is only for short testing.

One may change the loop from 1:3 to 1:42, to drive the shape to more extreme echinocyte-like shapes with C=120. However, it is necessary to simulate with increased number of steps, e.g., nsteps=2500 or higher, and also the spectral order should be higher, say 15.\
The final result is saved to disk in the current folder. Text-output is redirected to file by the sink()-command.

The following command allows to analyse the recorded MMC in terms of curvature and energy at accepted Montecarlo steps, sampled along the trajectory in a data frame M\$Sample. As third column, M\$Sample\$Cnt records the number of Metropolis rejections + 1 at the same energy. This is useful to evaluate thermodynamic ensemble expectation values of the recorded area, curvature and energy. For example, M\$Sample could allow for free-energy-perturbation analysis.

```{r plot, R.options = list(width=900,height=900)}
...
It should be noted that within this approach, $C_0$ has no effect on shape, since the energy contribution $C_0\ C$ remains constant. In `PSDC` the effect of the curvature gradient involving $C_0 \nabla C$ should be partially canceled out by the $C$ constraint gradient $-2 \rho \nabla C$ , where $\rho$ is the penalty constant `M.rho`.
...
The reference stomatocyte shape for C0=-6.5 was iterated at room temperature (kT=0.00411 aJ) for 100 steps under approximately constant curvature C=70.
...
```{r stomatocyte reduced model, R.options = list(width=900,height=900)}
...
```{r imagequnatity2d,R.options = list(width=900)}
...
L_MMC<-parallel::clusterEvalQ(cl,M<-MMC(M4,30))
# save results
...
A Newton minimization of a pre-computed stomatocyte shape of $l\le10$ goes like this ( $C_0=-3$ ):

```{r CNM}
data("S10_3");
SetParams(S10_3) # remember to set C0 and others correctly

CNM(S10_3,5,thresh=1e-3,plt=FALSE)->S10_3_cnm
plot(S10_3_cnm$E_CNM)
```

The constraint values for the `CNM()` need to be stored in the basis of the membrane object as `S10_3$bas$Target` before. From `CNM`, the Lagrangian multipliers are given in `S10_3_cnm$Lambda`. The text output from `CNM` gives information on the convergence in energy and Lagrangian multiplier $\lambda$. CNM exits if \|delta\| reported in output text is below CNM-parameter `Mtol_Newton`.

It is important to notice that inversion of Hessian in CNM may produce large jumps in coefficients, due to degeneracy (zero or almost zero eigenvalues). Therefore the pseudoinverse is applied, and directions of Hessian with absolute eigenvaules below CNM-parameter `threshold` are removed from the inverse. We have observed several times that negative eigenvalues appear with absolute value below 1e-4, leading to divergence. These are successfully suppressed by setting `threshold=1e-4`. However, one should take care to to suppress directions that do not correspond to degeneracy. One should therefore reduce the threshold more and more when recalling `CNM()`. CNM returns the eigensystem of the constraint Hessian as attr(M\$A,"Eigs"). The first two eigenvalues correpond to the Jacobian of constraint functions, in our notation negative.
...
# one may be insterested to see the effects -
...
For completeness, we also include data for lipid vesicle shape studies, like the reference shape Undustick (see <https://zenodo.org/records/13627757>).

SEN and area difference elasicity are de-activated by setting moduli to zero.

`data_MemRBC("US_L17_minim"); SetParams(US_L17_minim)`

`plot(US_L17_minim)`

`data_MemRBC("US_L17_rotated");SetParams(US_L17_rotated)`

`plot(US_L17_rotated)`

`# not yet finalized Undustick, de-novo from MemRBC: data_MemRBC("US_L10_de_novo")`

![Result of CNM on rotated Undustick membrane of area 140nm\^2 (basis reduced to $m\le1$ ) with $%l\le17$ loaded by `data("U17R"); SetParams(U17R); PSD(U17R,100)->U17R`. The shear stress parameters displayed did not enter energy. ](images/clipboard-3669985927.png)

The `unlist(Quantities(U17R))` reported for the CNM-results (preserved in data("u17rr")) and the Lagrangian multipliers $\lambda_{A,V}$ are:

```         
     Area    Volume      Curv        Wb  
     139.99999  85.49325 130.41239 -54.19321 
     
     la_A: -0.232144  la_V: 2.959906
```

Area, volume and curvature correspond to the Undustick reduced quantities a=1. v=0.55, c=1.55.
