...
-   **`M.Rcpp_ncores`** (\>0) number of CPU cores to use in parallel gradient and Hessian.
...
Users should check regularly that - besides SEN parameters - `M.C0` and constraint values in `M$bas$Target` is set correctly for their intended App usage. That's why most Apps report `C0` in their regular text output.
...
One may change the loop from 1:3 to 1:42, to drive the shape to more extreme echinocyte-like shapes with C=120. However, it is necessary to simulate with increased number of steps, e.g., nsteps=2500 or higher, and also the spectral order should be higher, say 15, or 18, like in provided data `M18_SDEx`, x=1...8.\
