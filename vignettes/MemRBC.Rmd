...
You load global standard parameters with `load_param_MemRBC()`. There is a demo dataset `M1` you can load and inspect. If you want the parameters from the membrane object `M1` to be set globally for further processing, use `SetParams(M1)` after `data(M1)`.
...
```{r CNM}
data("S4");
SetParams(S4) # remember to set C0 and others correctly
S4
CNM(S4,5,thresh=1e-3,plt=FALSE)->S4_cnm
plot(S4_cnm$E_CNM/M.Es,ylab=expression(E[CNM]),xlab="CNM iteration")
```
...
For completeness, we also include data for lipid vesicle shape studies, like the reference shape Undustick (see <https://zenodo.org/records/13627757>). The original $l\le17$ data is rotated by pi/2 for less mesh distortion, and constraints set to $A=4\pi$ and $V=0.55\ 4\pi/3$ (see `data(U17R)`). Furthermore, the basis was reduced to leading $v$-order $m\le1$ by `ReduceM1()`. The result can be loaded with `data(U17RR)`.

In the objects `U17R` and `U17RR`, SEN and area difference elasticity are de-activated by setting moduli to zero. Furthermore, $K_b=2$ is set (`M.K_b`) to have the original energy reporting (with energy scale `M.Es=16*pi)`.

```{r reduceBasisM1}
data("U17R"); SetParams(U17R) 
ReduceM1(U17R)->U17RR
Energy(U17RR)
Quantities(U17RR)
unlist(U17RR$Params)
