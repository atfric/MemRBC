...
# Only run this locally to generate data
run_it <- identical(Sys.getenv("NOT_CRAN"), "TRUE")
...
The core of MemRBC is a spectral numerical model of cellular shape, based on spherical harmonics. This means that the membrane shape is represented as real-valued coefficients $A$ of a series of spherical harmonics. It is important to know that $A$ as well as gradients of energy $G$ have 3 columns, each for a spatial dimension. Thus, the matrix of spatial points $X$ can be computed by three matrix-vector products as $X_s=Y*A_s$ , where column index $s\in \{1,2,3\}$ is the spatial dimension index for (x,y,z), and $Y$ are the precomputed spherical harmonics for spherical coordinates $(u_i,v_i)$ covering the 2D angular space $(0...\pi,0...2\pi)$. The number of spatial points $i=1...N$ is dictated by the integration scheme. The standard scheme implemented here is a Gauss-Legendre-Simpson scheme, in which the $v-$ angle is discretized equidistantly by Simpson rule, and $u$ are given at Gauss-Legendre points. The latter scheme avoids spatial points at the north and south pole. In consequence, in 3D surface plots one may locate the two poles as tiny holes.
...
The energy functional and its gradient are computed in spherical harmonics space, based on differential geometry first and second fundamental, evaluated at the integration points. The Hessian of energy is computed as forward finite difference of the gradient.

## Data in objects

A membrane object, also named MemRBC object, is of class MemRBC. It contains a coeficient object \$A (class MemA), a grid object \$grd (class MemGrd), a basis object \$bas (class MemBas) and possibly a coordinate object \$C (class MemC). Within \$C you may find a rgl mesh3d object \$Obj.

## Numerical Methods

Here we list the currently implemented numerical Apps and their features.

Apps enabling curvature constraint ("C") require the target curvature to be specified as `curv=value.`
...
-   **`M.Rcpp`** boolean (`TRUE` or `FALSE`) to use C++-Codes, needed for multi-core parallel codes

-   **`M.Rcpp_ncores`** (\>0) number of CPU cores to use in SCM energy and gradient. Parallel Hessian is done via a cluster, see parameter Mem_mc.cores to `FullModelHessian_Par()`.

-   not to be modified by the user are **`M.muk, M.lam`,** which are Augmented Lagrangian parameter $\mu$ and $\lambda$, used in `ALM()`

-   SEN parameters in the environment `MemRBC_dev` are `M.a2, M.a3, M.a4, M.b0, M.b1, M.b2.` Usually `M.a2` and `M.b0` are 1, but one may wish to change the leading order terms alone by setting other values.

Users should check regularly that - besides SEN parameters - `M.C0` and constraint values in `M$bas$Target` are set correctly for their intended App usage. That's why most Apps report `C0` in their regular text output.

## Demonstration

The data for the demonstrations in the vignette and help-examples must be obtained separatey from ZENODO: <https://zenodo.org/records/18667916>

They can be downloaded and installed into the packages data folder (after installing the package) with `library(MemRBC);get_data_ZENODO()`

You load global standard parameters with `load_param_MemRBC()`. There is a demo dataset `M4` you can load and inspect. If you want the parameters from the membrane object `M4` to be set globally for further processing, use `SetParams(M4)` after `data(M4)`.

```{r, start, echo = TRUE, eval = run_it}
library(MemRBC)
...
M4$bas$Target["Volume"]=95
save_MemRBC(M4,"M4_v95.rdat") # saves object with modified $Target
save_MemRBC(M4,"M4_v95_full_basis.rdat",reduce_basis = FALSE) # saves object with modified $Target and full basis, i.e. basis need not be recomputed on load_MemRBC.

```

```{r, plotfirst,webgl=TRUE, eval = run_it}
...
A standard membrane of spectral order 9 has now been loaded as `M1`. Parameters of `M` are shown and the shape is plotted in 3D.

Initial curvature C is reported to be 81.01442, and the 1056 integration points are on a 32 x 33 grid. The stored coefficients are in `M$A`, and `M$LA` is a list ready for keeping coefficients of further simulation steps.

```{r, prepareMMC, message=TRUE, warning=TRUE, webgl=TRUE, eval = run_it}
MakeStandardRBC(L=7,prn=TRUE,plt=TRUE)->M
rgl::rglwidget(width=300,height=250)
```

```{r MMCCcompute, webgl=TRUE, eval = run_it}
data(M4)
M=M4
...
A set of 3 curvature constraint Metropolis-Montecarlo simulations is performed with increase of target curvature by 1 unit per cycle. in `M$LA` every 10 accepted steps, coefficients are recorded. `nsteps=100` steps is only for short testing in vignette creation.

One may change the loop from 1:3 to 1:42, to drive the shape to more extreme echinocyte-like shapes with C=120. However, it is necessary to simulate with increased number of steps, e.g., nsteps=2500 or higher, and also the spectral order should be higher, say 15, or 18, like in data `S18-C0-*.qs2`published on ZENODO.\
The final result is saved to disk in the current folder. Text-output is redirected to file by the `sink()-`command.

The following command allows to analyse the recorded `MMC` in terms of curvature and energy at accepted Montecarlo steps, sampled along the trajectory in a data frame `M$Sample`. As third column, `M$Sample$Cnt` records the number of Metropolis rejections + 1 at the same energy. This is useful to evaluate thermodynamic ensemble expectation values of the recorded area, curvature and energy. For example, `M$Sample` could allow for some free-energy-perturbation analysis.

```{r plot,fig.dim = c(6, 6), eval = run_it}
PlotSample(M4) # can have differente colors for multiple runs
...
```{r plotM12, webgl=TRUE, eval = run_it}
rgl::clear3d();
plot(M12)
rgl::rglwidget(width=300,height=300)  
```

The reference stomatocyte shape for C0=-6.5 was iterated at room temperature (kT=0.00411 aJ, a stands for atto) for 100 steps under approximately constant curvature C=70.

A plot of a series of shapes is generated with `PlotLSeries` (truncation plot):

```{r LSeries, webgl=TRUE, echo=FALSE, eval = run_it}
...
```{r stomatocyte reduced model, fig.dim = c(6, 4), eval = run_it}

get_data_ZENODO(L="L5_stomatocyte_equilib.rda")
data("L5_stomatocyte_equilib") # data available at ZENODO
SetParams(L5_stomatocyte_equilib)
L5_stomatocyte_equilib -> M # sets C0 for next computations
MemRBC$M.Rcpp=TRUE
MemRBC$M.Rcpp_ncores=3
...
Energy(Mpsd)["E"]/MemRBC$M.Es-E0/MemRBC$M.Es
Energy(Mpnem)["E"]/MemRBC$M.Es-E0/MemRBC$M.Es

Mpsd$proc_time-M$proc_time
Mpnem$proc_time-M$proc_time

plot(last(Mpnem$E_total_PNEM-Mpnem$E_kin_PNEM,250)/MemRBC$M.Es,type="l",col=2,xlab="steps",ylab="E")
points(last(Mpsd$E_PSD/MemRBC$M.Es,250),type="l")
...
```{r plots, webgl=TRUE, eval = run_it}
rgl::open3d();
...
```{r imagequnatity2d,fig.dim = c(6, 3),, eval = run_it}
get_data_ZENODO(L="L9_Stomatocyte_6.rda")
data(L9_Stomatocyce_6); SetParams(L9_Stomatocyce_6)
  # you can get the data from ZENODO
...
startup<-parallel::clusterEvalQ(cl,MemRBC$M.C0<-c(-6,-4,-2,0)[tid])
#report that C0 is set
unlist(parallel::clusterEvalQ(cl,MemRBC$M.C0))
#perform only 30 mmc steps; 
M4$C0=MemRBC$M.C0
L_MMC<-parallel::clusterEvalQ(cl,M<-MMC(M4,30,C0=MemRBC$M.C0))
# save results
...
```{r CNM, fig.dim = c(6, 4), eval = run_it}
data("S4");
SetParams(S4) # remember to set C0 and others correctly, e.g. to MemRBC$M.C0
S4
CNM(S4,5,plt=FALSE)->S4_cnm
plot(S4_cnm$CNM_data[,1]/MemRBC$M.Es,ylab=expression(E[CNM]),xlab="CNM iteration")
...
```{r rotate,webgl=TRUE, eval = run_it}
...
```{r StomatoRotDiffm,webgl=TRUE , eval = run_it}
...
For completeness, we also include data for lipid vesicle shape studies, like the reference shape Undustick (see <https://zenodo.org/records/13627757>). The original $l\le17$ data is rotated by pi/2 for less mesh distortion, and constraints set to $A=4\pi$ and $V=0.55\ 4\pi/3$ (see `data(U17R)`).
